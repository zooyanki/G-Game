...
export function createTextures(scene: Phaser.Scene) {
  if (!scene.textures.exists('hero-sword-up')) {
    ...
    paint(scene, 'marker', 8, 8, (g) => {
      g.fillStyle(0xf4f0e6);
      g.fillRect(3, 0, 2, 8);
      g.fillRect(0, 3, 8, 2);
    });
    paint(scene, 'amulet', 8, 8, (g) => {
      g.fillStyle(0x2a5278);
      g.fillRect(2, 1, 4, 6);
      g.fillStyle(0xf2d48a);
      g.fillRect(3, 2, 2, 2);
    });
  ...
  if (!scene.textures.exists('chest-closed')) {
    paint(scene, 'chest-closed', 16, 16, drawChestClosed);
    paint(scene, 'chest-open', 16, 16, drawChestOpen);
  }
}
...
function drawChestClosed(g: Phaser.GameObjects.Graphics) {
...
function drawChestOpen(g: Phaser.GameObjects.Graphics) {
  g.fillStyle(0x5c3317);
  g.fillRect(2, 8, 12, 6);
  g.fillStyle(0x8a5a32);
  g.fillRect(1, 3, 14, 5);
